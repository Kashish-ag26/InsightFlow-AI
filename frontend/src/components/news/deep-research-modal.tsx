...
export function DeepResearchModal({ isOpen, onClose, article }: DeepResearchModalProps) {
  ...
  // Dynamic Domain & Article Personalized Mechanics Generator (Strict Category Prioritization)
  const getDomainBreakdown = () => {
    const title = cleanTitle.toLowerCase();
    const cat = article.category.toLowerCase();
    const source = article.source;

    // Check CATEGORY FIRST to ensure perfect domain alignment
    ...
    if (cat.includes("war") || cat.includes("defense")) {
    ...
    if (cat.includes("stock") || cat.includes("finance")) {
    ...
    if (cat.includes("fashion") || cat.includes("lifestyle")) {
    ...
    if (cat.includes("industry") || cat.includes("energy")) {
    ...
    // Fallback keyword checks if category string is unmapped
    if (title.includes("farmer") || title.includes("parliament") || title.includes("election")) {
      return `Legislative Policy & Reform Mechanics: Operational assessment of "${cleanTitle}" reveals key regulatory adjustments reported by ${source}.`;
    }
    if (title.includes("ai") || title.includes("claude") || title.includes("model")) {
      return `System Architecture & Algorithmic Mechanics: Technical analysis of "${cleanTitle}" details GPU/TPU pipeline optimizations validated by ${source}.`;
    }
  ...
  // Dynamic Socio-Economic Impact Generator (Strict Category Prioritization)
